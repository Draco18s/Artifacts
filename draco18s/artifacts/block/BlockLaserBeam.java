...
public class BlockLaserBeam extends Block {
	...
	public BlockLaserBeam(int par1) {
		super(par1, Material.circuits);
		this.setTickRandomly(true);
        this.setUnlocalizedName("Laser Beam");
        this.setTextureName("artifacts:laser");
	}
            ...
            while (i1 < 4)
            {
                ...
                while (true)
                {
                    if (j1 < 16)
                    {
                        ...
                        if (i2 == BlockLaserBeamSource.instance.blockID)
                        {
                        ...
                        else if (i2 == this.blockID || !par1World.isBlockOpaqueCube(k1, par3, l1))
                        {
                            ++j1;
                            continue;
                        }
        ...
        while (i1 < 4)
        {
            ...
            while (true)
            {
                if (j1 < 16)
                {
                    ...
                    int i2 = par1IBlockAccess.getBlockId(k1, par3, l1);

                    if (i2 == BlockLaserBeamSource.instance.blockID)
                    {
                        flag[i1] = true;
                    }
                    else if (i2 == this.blockID || !par1IBlockAccess.isBlockOpaqueCube(k1, par3, l1))
                    {
                        ++j1;
                        continue;
                    }
	...
	public static boolean func_72148_a(IBlockAccess world, int x, int y, int z, int l, int i) {
        ...
        int l1 = world.getBlockId(j1, y, k1);
        boolean flag = (l & 2) == 2;
        
        int i2;

        if (l1 == BlockLaserBeamSource.instance.blockID)
        {
        ...
        else if (l1 == BlockLaserBeam.instance.blockID)
        {
            i2 = world.getBlockMetadata(j1, y, k1);
		...
		while (i1 < 4)
        {
            ...
            while (true)
            {
                if (j1 < 16)
                {
                    ...
                    int i2 = world.getBlockId(k1, y, l1);
                    ...
                    if (i2 == BlockLaserBeamSource.instance.blockID) {
                    	...
                    	world.scheduleBlockUpdate(k1, y, l1, BlockLaserBeamSource.instance.blockID, BlockLaserBeamSource.instance.tickRate(world));
                    }
                    else if (i2 == this.blockID) {
                    	world.setBlockMetadataWithNotify(k1, y, l1, world.getBlockMetadata(k1, y, l1)|1, 3);
                    	++j1;
                        continue;
                    }
                    else if(!world.isBlockOpaqueCube(k1, y, l1))
                    {
                        ++j1;
                        continue;
                    }
